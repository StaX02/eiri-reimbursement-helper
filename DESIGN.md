...
报销单详情使用所有者窗口中的独立 WPF 窗口；复用 App.xaml 的输入、按钮、表格和滚动条样式及 ThemeManager 主题资源。申请日期使用 yyyy-MM-dd 文本输入；标签关联输入框。保存后留在详情，关闭后刷新列表；有未保存修改时提示。状态消息在窗口底部显示，操作期间禁用编辑和重复导入。附件提供拖入和文件选择按钮；双击或 Enter 打开。金额为空与零明确区分，重复识别仅填空字段。
