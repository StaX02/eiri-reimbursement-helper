...
审批流程状态是报销单属性的最后一项，只读可复制，与列表“流程”列共享显示规则：未提交、审批中、已撤销、审批完成；已提交但没有实例时显示“无审批实例”，尚未获取状态时显示“待获取”。报销单列表标题栏右侧和详情“钉钉提审”右侧的“刷新流程”共用同一命令，均刷新全库符合条件的未归档报销单。启动时自动刷新；进行中禁用重复刷新，保留列表选择、分页和详情输入。获取失败保留上次状态，在主窗口状态区显示数量、对应条目和重试指引；长消息在最高 96px 的区域内滚动。归档窗口保留状态展示并隐藏刷新入口。
