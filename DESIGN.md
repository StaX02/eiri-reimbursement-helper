...
主窗口与归档窗口最大化时，外缘对齐当前显示器的工作区，避开任务栏；使用 Win32 工作区像素边界处理不同缩放比例，还原后恢复原窗口位置和尺寸。
...
报销单列表在总金额后显示“已导出”“已提交”“流程”“已返款”，统一使用 104px 列宽；日期使用数据文本样式，流程使用正文样式。里程碑已完成时显示 yyyy-MM-dd 日期，未完成时分别显示“未导出”“未提交”“未返款”。

审批流程状态是报销单属性的最后一项，只读可复制，与列表“流程”列共享显示规则：未提交、审批中、已撤销、已同意、已拒绝；已提交但没有实例时显示“无审批实例”，尚未获取状态时显示“待获取”；旧记录已完成但缺少结果时显示“结果待获取”并在刷新时补查。已拒绝或已撤销允许打开提审表单修改后重新提交。重新提审保留旧实例至成功，结果不明时继续要求核对；成功后替换实例并清空旧状态与结果。报销单列表标题栏右侧和详情“钉钉提审”右侧的“刷新流程”共用同一命令，均刷新全库符合条件的未归档报销单。启动时自动刷新；进行中禁用重复刷新，保留列表选择、分页和详情输入。获取失败保留上次状态，在主窗口状态区显示数量、对应条目和重试指引；长消息在最高 96px 的区域内滚动。归档窗口保留状态展示并隐藏刷新入口。
...
“提交报销审批”在流程就绪后启用，点击后完整校验并构造请求。提交期间禁用编辑及重复提交并阻止关闭；成功后显示结果，不在提审窗口显示实例 ID。实例 ID、审批编号、审批流程状态依次位于报销单详情“报销单属性”中，只读可复制。审批编号随流程刷新保存，未获取时留空，长编号自动换行；重新提审成功后清空旧编号。结果不明保留待核对状态，本地保存失败仅重试保存回执。审批查询和表单加载遵循关闭取消规则。
...
顶端菜单顺序为“钉钉、归档、选项、关于”；“归档 → 查看归档”打开主窗口所属的独立模态窗口。归档窗口复用 MainWindow 的列表、互斥选择、分页与详情，并继续使用 ReimbursementDetailView、App.xaml 和 ThemeManager。三项里程碑全部完成的订单及报销单在主窗口隐藏，在归档窗口展示。归档中隐藏新增、导入、拖放及提审入口；属性文本框只读且可复制，发票选择、详情选项卡、附件双击和 Enter 查看保持可用。归档报销单右键菜单仅提供“取消归档”，支持多选；操作期间禁用重复操作，失败保留条目并显示错误，成功同步刷新归档与主窗口并清除失效选择。最后一页清空后回退至上一页，空列表说明自动归档条件。
...
已同意报销单的“导出报销资料”进入审批 PDF 流程：单选使用 PDF 保存对话框，多选只选择一次目录，自动命名并避让同名文件。混合选择时普通报销单复用该目录；审批 PDF 出错时保留错误提示。生成期间复用 IsBusy 禁用重复操作。首页白色 A4、黑色宋体，标题居中小三号（15pt），表格居中占页宽 80%、列宽 20%/80%，正文小五号（9pt）自动换行；内容超出首页容量时提示调整，保持字号且不截断。图片等比居中填充横/竖 A4 页面，完整展示。PDF 已生成但状态写入失败时明确提示，保留文件。
