...
钉钉菜单位于“选项”前，包含“连接接口”和“清除连接记录”。连接弹窗沿用 WPF 标签、输入、按钮和主题资源；Client Secret 使用 PasswordBox，AccessToken 使用可选择复制且自动换行的只读文本框。请求期间禁用输入和确定按钮，关闭弹窗取消请求；失败留在弹窗中供修正和重试，成功保存后显示令牌。清除沿用所有者窗口确认框，默认选择“否”，成功消息显示在主窗口状态栏。
