...
5. 文本层中的发票明细根据每页“项目名称”“规格型号”和“合计”标签的坐标，先裁取项目名称列，再按行顺序拼接换行内容；规格、数量及金额列不进入名称。找不到这些表头的布局与 OCR 输入继续使用已有文本解析。
6. 缺失、冲突或低置信度字段标记 `needs_review`。
...
- Desktop：在视图模型边界验证导入、校正、导出、归档及提审的关键行为；WPF 仅检查窗口加载、选择状态和归档只读绑定。主题、尺寸、文案和附件打开人工验收，不在自动测试中重复弹窗或生成截图。
- 精简原则：保留数据丢失、重复提交、错误金额和事务一致性的回归保护；删除纯展示断言、相同分支的重复输入和可由小夹具覆盖的大量造数。
